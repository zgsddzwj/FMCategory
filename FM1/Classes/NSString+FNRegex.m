...
@implementation NSString (FNRegex)
#pragma mark - 判断全汉字
...
- (BOOL)bankCardluhmCheck{
    ...
    NSMutableArray * arrEvFNum = [[NSMutableArray alloc] initWithCapacity:0];//偶数位数组
    
    for (int i=0; i< forwardDescArr.count; i++) {
        NSInteger num = [forwardDescArr[i] intValue];
        if (i%2) {//偶数位
            [arrEvFNum addObject:[NSNumber numberWithInteger:num]];
        }else{//奇数位
            if (num * 2 < 9) {
    ...
    __block NSInteger sumEvFNumTotal =0 ;
    [arrEvFNum enumerateObjectsUsingBlock:^(NSNumber * obj, NSUInteger idx, BOOL *stop) {
        sumEvFNumTotal += [obj integerValue];
    }];
    
    NSInteger lastNumber = [lastNum integerValue];
    
    NSInteger luhmTotal = lastNumber + sumEvFNumTotal + sumOddNum2Total + sumOddNumTotal;
